...
使用 `scripts/generate_pdf.py` 生成 `docs/submission.pdf`，内容包含 GitHub 仓库地址和 README 关键段落。

```bash
python scripts/generate_pdf.py --repo https://github.com/<your-account>/AI-Travel-Planner
```

> 提示：若需保留 README 中的中文字符，请下载 `NotoSansSC-Regular.otf` 并放置到 `fonts/` 目录，脚本会自动使用该字体生成完整中文 PDF。

助教可直接查阅 PDF 了解仓库地址及运行说明。

## 📬 下一步建议

1. 对接真实地理编码服务，为行程中的景点自动补足经纬度。
2. 增加多用户协作或评论功能，实现家庭/团队共创行程。
3. 引入单元测试与端到端测试，提升代码质量与部署信心。
4. 扩展预算分析，结合历史消费数据生成更精准的支出预测。

---

如需更多帮助，可在设置页面查看配置提示，或在 README issue 中提出问题。祝旅程愉快、项目顺利！🧳✈️
